...
张量：
    PyTorch框架属于最常用的深度学习框架，无论是后续要学的ANN，还是CNN，RNN
    底层在处理数据时，都是使用张量来处理的
...
def dm03():
    ...
    # 场景3：numpy nd数组 -> 张量
    data = np.random.randint(0, 10, size=(2, 3))
    t3 = torch.IntTensor(data)      # 默认int32
    print(f"t3:{t3},type:{type(t3)}")
    print("-" * 30)

    # 场景4：如果类型不匹配，会尝试自动转换类型
    data = np.random.randint(0, 10, size=(2, 3))
    t4 = torch.FloatTensor(data)    # 将int自动转换为float，默认：float32
    print(f"t4:{t4},type:{type(t4)}")
