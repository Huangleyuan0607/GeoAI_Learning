...
def dm01():
    ...
    # lr = 0.01      # 较小的学习率会导致梯度下降速度慢
    # lr = 0.1      # 正常的梯度下降
    lr = 0.125    # 当学习率设置在0.125，一下子求出一个最优解
    ...
    for i in range(max_iteration):
        ...
        x_rec.append(x.item())      # 梯度下降点列表

        # 更新参数
        x.data.sub_(lr * x.grad)        # x = x - lr * x.grad
        x.grad.zero_()
        iter_rec.append(i)              # 迭代次数列表
        loss_rec.append(y.item())       # 损失值列表，这里将y改成y.item()以获取标量值
