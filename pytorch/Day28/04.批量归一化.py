"""
案例：
    代码演示 批量归一化，它也属于正则化的一种，用于缓解模型过拟合的情况

批量归一化：
    思路：
        先对数据做标准化（会丢失一些信息），然后对数据做缩放（λ，理解成：w权重） + 平移（β，理解成：b偏置），再找补回一些信息；
    应用场景：
        在计算机视觉（CV）领域使用较多
...
def dm01():
    ...
    # 参4：表示使用可学习的变换参数(λ和β)，对归一化（标准化）后的数据进行缩放和平移
    bn_2d = nn.BatchNorm2d(num_features = 2, eps = 1e-5, momentum = 0.1, affine = True)        # 图像是几通道，输入特征就是几
