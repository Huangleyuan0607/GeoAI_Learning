...
多分类交叉熵损失函数：CrossEntropyLoss
    ...
    细节：
        CrossEntropyLoss = Softmax() + 损失计算，后续如果用这个损失函数，则：输出层就不用额外调用softmax()激活函数了
"""
...
def dm01():
    ...
    y_true = torch.tensor([[0, 1, 0], [1, 0, 0]], dtype = torch.float)      # 处理前
    # y_true = torch.tensor([1, 2])         # 处理后

    # 2.手动创建样本的预测值 -> 上述公式中的f(x)
