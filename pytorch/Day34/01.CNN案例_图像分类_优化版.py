...
class ImageModel(nn.Module):
    ...
    def __init__(self):
        ...
        """优化8：增加卷积层和池化层"""
        # 卷积层3,：输入16通道，输出32通道，卷积核大小3x3，步长1填充0（默认）
        ...
        """优化1：增加全连接层参数量"""
        # 隐藏层（全连接层）1：输入32*2*2，输出256
        self.fc1 = nn.Linear(32 * 2 * 2, 256)
        # 隐藏层（全连接层）2：输入256，输出128
    ...
    def forward(self, x):
        ...
        # 细节：全连接层只能处理二维数据，所以要将数据进行展平处理 (N, 32, 6, 6) -> (N, 32 * 6 * 6)
...
def train(train_dataset):
    ...
    # 6.保存模型
    torch.save(model.state_dict(), './pytorch/Day34/model/image_model_new.pth')
...
def evaluate(test_dataset):
    ...
    model = ImageModel().to(device)
    # 3.加载模型参数
    # model.load_state_dict(torch.load('./pytorch/Day34/model/image_model_new.pth'))      # pickle文件
    """优化7：使用GPU进行训练"""
    model.load_state_dict(torch.load('./pytorch/Day34/model/image_model_new.pth', map_location=device))
    # 4.定义变量，统计预测正确的样本个数以及总样本个数
    total_correct, total_samples = 0, 0
    # 5.遍历数据加载器，获取每批次的数据
    """优化5：增加with torch.no_grad():，测试时不需要计算梯度，节省显存和计算"""
    with torch.no_grad():
